...
# Import Keras 3.x compatibility layer for legacy models
...
if os.path.exists(code_dir):
    ...
    try:
        ...
        KERAS3_COMPAT_AVAILABLE = True
    except ImportError:
        KERAS3_COMPAT_AVAILABLE = False
else:
    KERAS3_COMPAT_AVAILABLE = False
...
class NeuralNetwork:
    ...
    def load(self, file_path, custom_objects={}):
        ...
        """
        # Try loading with Keras 3.x compatibility layer first (for legacy models)
        loaded = False
        if KERAS3_COMPAT_AVAILABLE:
            try:
                self.model = load_model_keras3_compat(file_path, custom_objects, compile=False)
                loaded = True
            except Exception as e:
                # Fall back to standard load_model if compatibility layer fails
                print(f"Warning: Keras 3.x compatibility layer failed, trying standard loader: {e}")

        # Fall back to standard Keras load_model
        if not loaded:
